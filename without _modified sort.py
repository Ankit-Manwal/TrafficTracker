...
mask= cv2.imread('images and videos/mask3.png')   # crop portion of frame, region  only where detection work
mask = cv2.resize(mask, (1280,720))  #mask = cv2.resize(mask, (img.shape[1], img.shape[0])) after cap.read
...
while True:
    ...
    detections=np.empty(shape=(0, 5)) # initially 0  & each row 5 elements [x1, y1, x2, y2, confidence]
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            if (curr_class in ["car", "truck", "bus"]) and confidence > 30:
                ...
                cv2.putText(img=frame, text=f'   {coco_classes[curr_class_index]}', org=(max(0, x1), max(35, y1 - 20)),
    ...
    for result in resultsTracker:
        ...
        if speed_kph!=0:
            cv2.putText(img=frame, text=f'{ID}    {int(speed_kph)} km/h', org=(max(0, x1), max(35, y1 - 20)),
                fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(0, 0, 255), thickness=2)
        else:
            cv2.putText(img=frame, text=f'{ID}', org=(max(0, x1), max(35, y1 - 20)),
                fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(0, 0, 255), thickness=2)
